...
// Max.
contract Max is ERC20('Maximum Max Token', 'MAX') {
...
contract MaxMasterChef is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    event MaxMintError(bytes reason);
    ...
    function pendingMax(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && pool.lpSupply != 0 && totalAllocPoint > 0) {
            ...
            accMaxPerShare = accMaxPerShare.add(MaxReward.mul(1e18).div(pool.lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        try token.mint(devaddr, MaxReward.div(10)) {
        } catch (bytes memory reason) {
            MaxReward = 0;
            emit MaxMintError(reason);
        }

        try token.mint(address(this), MaxReward) {
        } catch (bytes memory reason) {
            MaxReward = 0;
            emit MaxMintError(reason);
        }

        pool.accMaxPerShare = pool.accMaxPerShare.add(MaxReward.mul(1e18).div(pool.lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    // Safe Max transfer function, just in case if rounding error causes pool to not have enough ALPHAs.
    function safeMaxTransfer(address _to, uint256 _amount) internal {
        uint256 MaxBal = token.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > MaxBal) {
            transferSuccess = token.transfer(_to, MaxBal);
        } else {
    ...
    function updateEmissionRate(uint256 _MaxPerBlock) external onlyOwner {
        require(_MaxPerBlock <= MAX_EMISSION_RATE, "INCORRECT INPUT");
        massUpdatePools();
        MaxPerBlock = _MaxPerBlock;
